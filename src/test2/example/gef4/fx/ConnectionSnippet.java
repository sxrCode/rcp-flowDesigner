...
public class ConnectionSnippet extends AbstractFxExample {
	...
	public static class startStar extends Polygon {
		public startStar() {
			super(-5, -3, 3, 5, 3, -5, -5, 3, 7, 0);
			setFill(Color.WHITE);
			setStroke(Color.RED);
			setStrokeLineJoin(StrokeLineJoin.ROUND);
		}
		...
		// applications on Mac OS X El Capitan
		{
			...
			if (System.getProperty("java.version").startsWith("1.7.0")
				...
				try {
					...
					Field psNameToPathMapField = macFontFinderClass.getDeclaredField("psNameToPathMap");
	...
	public Scene createScene() {
		...
		// use a shape
		Rectangle end2 = new Rectangle(200, 50, 30, 30);
		end2.setArcWidth(50);
		end2.setArcHeight(50);
		...
		Circle c1 = new Circle(150, 50, 30.0);
		...
		Connection c1Connection = new Connection();
		c1Connection.setEndDecoration(new ArrowHead());
		c1Connection.setStartAnchor(new DynamicAnchor(start, new OrthogonalProjectionStrategy()));
		c1Connection.setEndAnchor(new DynamicAnchor(c1, new OrthogonalProjectionStrategy()));
		c1Connection.setStartDecoration(new startStar());

		Group root = new Group();
		root.getChildren().addAll(start, end1, end2, connection1, connection2);
		root.getChildren().addAll(c1, c1Connection);
